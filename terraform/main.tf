# Terraform configuration for Rancher Deployment Platform
terraform {
...
provider "aws" {
  ...
  default_tags {
    tags = {
      Project     = "rancher-deployment-platform"
...
resource "aws_security_group" "rancher_server" {
  name_prefix = "${var.project_name}-rancher-server-"
  vpc_id      = aws_vpc.main.id
  ...
  tags = {
    Name = "${var.project_name}-rancher-server-sg-${var.environment}"
  }
}
...
resource "aws_security_group" "worker_nodes" {
  ...
  tags = {
    Name = "${var.project_name}-worker-nodes-sg-${var.environment}"
  }
}
